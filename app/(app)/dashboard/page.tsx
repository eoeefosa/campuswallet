...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [walletRes, budgetsRes, expensesRes, summaryRes] =
          await Promise.allSettled([
            api.get<{ balance: number }>("/wallet/balance"),
            api.get<Budget[]>("/budgets"),
            api.get<Expense[]>("/expenses?limit=5"),
            api.get<DashboardData>("/reports/monthly"),
          ]);
        ...
        if (expensesRes.status === "fulfilled")
          setRecentExpenses(expensesRes.value);
      ...
      {alertedBudgets.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 mb-8">
          <div className="text-sm font-semibold text-blue-800 mb-3 flex items-center gap-2">
            ...
            <h2> Budget Alerts ({alertedBudgets.length})</h2>
          </div>
              ...
              return (
                <div key={b.id}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span
                      className={
                        isOver ? "text-blue-600 font-semibold" : "text-blue-700"
                      }
                    >
                  ...
                  <div className="h-2 bg-blue-200 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${isOver ? "bg-blue-500" : "bg-blue-400"}`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
          ...
          <Link
            href="/budgets"
            className="text-blue-700 text-xs mt-3 inline-block hover:underline font-medium"
          >
            Manage budgets →
          </Link>
        </div>
